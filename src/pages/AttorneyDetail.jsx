    ...
    <main className="pa-page" role="main">
      ...
      <section className="pa-hero pa-hero-attorney" aria-label="כותרת עמוד">
        ...
        <div className="pa-hero-inner">
          ...
          <p className="pa-hero-sub">ברזילי, עזורי ושות׳ עורכי דין</p>
        </div>
      </section>
      ...
      <div className="pa-content">
        ...
        {/* Profile — asymmetric editorial layout: bled photo, offset column,
            hanging bio, marginal facts/contact rail */}
        <section ref={profileRef} className="atty-profile" aria-labelledby="profile-heading">
          <h2 id="profile-heading" className="sr-only">{attorney.name}</h2>

          <div className={`atty-profile-photo reveal-slide-left ${profileVisible ? 'active' : ''}`}>
            <img
              src={attorney.photo}
              alt={attorney.name}
          ...
          <div className={`atty-profile-body reveal-slide-right ${profileVisible ? 'active' : ''}`}>
            <div className="atty-profile-bio">
              {attorney.bio.map((para, i) => (
                <p key={i} className="atty-profile-lede">{para}</p>
              ))}
            </div>
          </div>

          <aside className={`atty-profile-rail reveal-fade-up ${profileVisible ? 'active' : ''}`} aria-label="פרטי התקשרות והשכלה">
            <h3 className="atty-rail-heading">פרטי התקשרות</h3>
            <ul className="atty-rail-contact">
              <li>
              ...
              <li>
                <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="atty-rail-wa">
            ...
            {attorney.languages && (
              <div className="atty-rail-facts">
                <h3>שפות</h3>
                <p className="atty-rail-oneline">{attorney.languages.join(' ו')}.</p>
              </div>
            )}

            <div className="atty-profile-cert atty-profile-cert--oneline">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" />
                <polyline points="22 4 12 14.01 9 11.01" />
              </svg>
              <span>מוסמך לעריכת ייפוי כוח מתמשך מטעם משרד המשפטים ולשכת עורכי הדין</span>
            </div>
          </aside>
        </section>
